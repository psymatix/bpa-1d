/*
 * Modified Subset sum problem solution using Backtracking techniques
...
function SubsetSum() {
    ...
    ssApp.chromosome = function(sequence) {
        ...
        if (sequence.length < 1 || sequence === '') {
            ...
            var seqStr = chromosome.sequence.toString(),
                seqArr, seqStrArr;
            seqStrArr = seqStr.split("");
            seqArr = seqStrArr.map(function(v) {
                return parseInt(v);
            });
        ...
        if (ssApp.testChromosomeSum(chromosome)) { // test 1
            ...
            if (ssApp.runningTotal(chromosome)) { // test 3 and sum augmentation
                ...
                chromosome.finalSum = chromosome.combination.reduce(function(previousValue, currentValue, index, array) {
    ...
    ssApp.testChromosomeSum = function(chromosome) {
        ...
        totSum = chromosome.seqArr.reduce(function(previousValue, currentValue, index, array) {
            return previousValue + currentValue;
        });
        ...
        for (i = 0; i < chromosome.seqArr.length; i++) {
            ...
            if ((runsum + chromosome.seqArr[i]) < ssApp.targetSum) {
                ...
                if (runsum === ssApp.targetSum) {
                    ...
                    chromosome.combination.sort(function(a, b) {
                        return a - b;
                    });
