...
var copyBinGroup = function(originalRef, copyRef){
    ...
    // originalRef and copyRef are arrays
                ...
                for(i=0; i<originalRef.length; i++){ 
                    var $b = originalRef[i],  b = bin($b.id,$b.size,$b.capacityUsed);
...
App.binSort = function(sorting, binGroup){
        ...
        // var bins = []; copyBinGroup(binGroup, bins); 
        var bins = binGroup.slice(); 
         ...
         var chosenBinId = bins[0].id, cb;
         
         var cbArr = binGroup.filter(function(elem){ 
                                    if(elem.id === chosenBinId){ return true; }
                                });
