...
App.init = function(config){
	...
	if(typeof config.bins !== 'undefined'){
		...
		for (var p in $cfgbins){
                ...
                App.originalBins = []; // have to recreate bin recursively
                for(i=0; i<App.bins.length; i++){ 
                    var $b = App.bins[i],  b = bin($b.id,$b.size,$b.capacityUsed);
                    b.position = $b.position;
                    App.originalBins.push(b);        
                    }
...
App.unpackInit = function(){
    ...
    for(i=0; i<App.originalBins.length; i++){ 
        var $b = App.originalBins[i],  b = bin($b.id,$b.size,$b.capacityUsed);
            b.capacityUsed = parseInt(b.size) - parseInt(b.capacityUsed);
            b.position = $b.position;
            App.inverseBins.push(b);        
        }
